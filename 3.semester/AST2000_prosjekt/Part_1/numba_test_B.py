    ...
    def simulate(self):
        ...
        def run():
            ...
            for i in range(1000):
                ...
                r[:,0,:] = r[:,0,:] + v[:,0,:]*dt
                # Vektorisering
                # a = np.logical_or(abs(r[:, 0, 0]>=L/2), abs(r[:, 0, 1]>=L/2))
                # v[a] *= -1
                # v[np.where(r[:, 0, 2]>=L/2)] *= -1
                # b = np.logical_and(r[:, 0 ,2]<=-L/2, np.logical_and(abs(r[:, 0, 0]) < nozzle_side/2, abs(r[:, 0, 1]) < nozzle_side/2))
                # c = np.logical_and(r[:, 0 ,2]<=-L/2, np.logical_and(abs(r[:, 0, 0]) >= nozzle_side/2, abs(r[:, 0, 1]) >= nozzle_side/2))
                # v[c] *= -1
                # if np.any(b):
                #     particles_out += len(v[b])
                #     rocket_p += H2_mass*np.sum(abs(v[b]))
                #     r[b] = [0, 0, L/2 - 1e-12]
                for j in range(N):
            ...
            return v, r, rocket_p, rocket_P/(6*T), particles_out/T, T
        ...
        self.rocket_F = self.rocket_p / self.T
        print('-------------------------------------------')
...
if __name__ == '__main__':
    engine = Box(len_box=0.5e-6, T=3.5e3, num_particles=250e3, nozzle_side_len=0.25e-6)  # Må kjøre på 100 000 !!!!!
    engine.simulate()
...
print(thrust_force)
print(mass_loss_rocket)
rotational_time = system.rotational_periods[0]*24*60*60

@njit(cache=True)
def rocket_boost(dv, fuel, time, gravity=False):
    if gravity == True:
        gamma = G
        print('gravity is on')
    if gravity == False:
    ...
    e_r = r[0] / r_norm
    a = thrust_force/total_mass*i - gamma*planet_mass / r_norm**2*e_r
    T = 0
    for i in range(time_steps-1):
        ...
        a = thrust_force/total_mass*e_r - gamma*planet_mass / r_norm**2*e_r
        T += dt
        if total_mass <= spacecraft_mass:
            ...
            print('r=', r[i+1])
            print('v=', v_norm)
            return t, fuel_consumption, total_mass - spacecraft_mass, v, r, T
        if r_norm < planet_radius:
            r[i+1] = planet_radius*e_r
        if v_norm >= v_escape:
            print('Sucessful launch in', T, 's')
            print('v=', v_norm)
            return t, fuel_consumption, total_mass - spacecraft_mass, v, r, T
    if T > time and gravity == True:
        print('Launch is taking too much time')
    print('final speed', v_norm, 'm/s')
    print('time : ', T)
    return t, fuel_consumption, total_mass - spacecraft_mass, v, r, T

t, mass_consumed_boost, fuel_left, v, r, T = rocket_boost(v_escape, initial_fuel, 1200, gravity=True)
...
print(r[np.where(r != 0)][-1])
R = r[np.where(r != 0)][-1] / AU + r_planet + system.initial_velocities[:,0]*T / (365*24*60*60)
...
print(g)
mission.set_launch_parameters(thrust_force, mass_loss_rocket, initial_fuel, 3*600, pos, 0)
