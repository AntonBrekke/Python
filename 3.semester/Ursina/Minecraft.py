    ...
    def input(self, key):
        if self.hovered:
            if key == 'right mouse down':
                ...
                if block_pick == 3: voxel = Voxel(pos=self.position + mouse.normal, texture=dirt_texture)
                if block_pick == 4: voxel = Voxel(pos=self.position + mouse.normal, texture=brick_texture)
...
for z in range(25):
    for x in range(25):
        voxel_top = Voxel(pos=(x,0,z))
        for y in range(-1, -2, -1):
            voxel = Voxel(pos=(x,y,z), texture=dirt_texture)
